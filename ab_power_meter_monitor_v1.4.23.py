...
def write_fits(
                        ...
                        if ext in result:
                            ...
                            for p, vals in old_cols.items():
                                if p in result[ext]["columns"]:
                                    ...
                                    n_existing = len(
                                        result[ext]["timestamps"]) - len(old_ts)
                                    result[ext]["columns"][p] = (
                                        [float("nan")] * n_existing + vals
                                    )
    ...
    for ip in all_device_data:
        ...
        if append and os.path.isfile(filename):
            ...
            main_readable = bool(disk_hdus) or True
            # _read_fits_to_dict returns {} on total failure AND on an
            # empty-but-valid file; disambiguate using a separate flag.
            # Re-check: if the file exists but _read_fits_to_dict returned
            # {} due to an exception it already logged a warning.
            # We mark readable=True only if no exception escaped the helper
            # (the helper always returns {} on error without raising).
            # Use a second probe: attempt a minimal open to confirm the
            # file is accessible at all.
            try:
...
def write_xlsx(
    ...
    _rescue_suffix = datetime.datetime.now().strftime("%Y%m%d_%H%M%S") + "_rescue"
