...
def run_headless(cfg: Dict[str, Any]) -> None:
    ...
    dicts_only = bool(cfg.get("headless_console_dicts_only",
                      HEADLESS_CONSOLE_DICTS_ONLY)) and not silent
    print_each = bool(cfg.get("headless_print_each_sample",
                      HEADLESS_PRINT_EACH_SAMPLE)) and not silent
    print_cumulative = bool(cfg.get(
        "headless_print_cumulative",    HEADLESS_PRINT_CUMULATIVE)) and not silent
