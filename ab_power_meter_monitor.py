...
combination of:
  ...
  • Appending columnar text log files (timestamp=row, parameter=column)
...
def accumulate_poll(all_device_data: Dict[str, Dict[str, Dict[str, Any]]]) -> None:
                ...
                if tname not in TIME_SERIES_STORE[ip]:
                    ...
                    for param in series:
                        if param not in existing:
                            ...
                            n_existing = len(TIME_SERIES_STORE[ip][tname]["timestamps_local"])
    ...
    logger.debug("Accumulator updated — %d devices, local ts: %s", len(all_device_data), local_ts)
...
def should_flush(cfg: Dict[str, Any]) -> bool:
    ...
    if store_mb > threshold:
        logger.info("Flush triggered: store size %.1f MB > threshold %.1f MB", store_mb, threshold)
        return True
    if free_mb < free_min:
                ...
                if is_new:
                    ...
                    with open(filename, "w", newline="", encoding="utf-8") as fh:
                        ...
                        for i, ts in enumerate(timestamps):
                            row = [ts] + [columns[p][i] if i < len(columns[p]) else "" for p in params]
                            writer.writerow(row)
                else:
                    ...
                    with open(filename, "r", newline="", encoding="utf-8") as fh:
                        ...
                        for row_idx, row in enumerate(reader):
                            if row_idx == 0:
                                existing_params = row[1:]  # skip Timestamp_Local
                            elif row_idx == 1:
                                pass  # units row
                            else:
                    ...
                    if new_params:
                        ...
                        with open(filename, "w", newline="", encoding="utf-8") as fh:
                            ...
                            for old_row in old_rows[2:]:
                                writer.writerow(old_row + [""] * len(new_params))
                    ...
                    with open(filename, "a", newline="", encoding="utf-8") as fh:
                        ...
                        for i, ts in enumerate(rows_to_write):
                            abs_i = start_idx + i
                            row   = [ts] + [
                                columns[p][abs_i] if (p in columns and abs_i < len(columns[p])) else ""
                                for p in all_params
                            ]
                            writer.writerow(row)
        ...
        for tname, tdata in tables.items():
            ...
            for i, ts in enumerate(timestamps):
                ...
                for col_idx, param in enumerate(params, start=2):
                    val = columns[param][i] if i < len(columns[param]) else None
            ...
            if n_data_rows >= 2 and numeric_cols:
                try:
                    ...
                    for nc in numeric_cols[:12]:   # cap at 12 series for legibility
                        data_ref = Reference(
                            ws,
                    ...
                    cat_ref = Reference(ws, min_col=1, min_row=3, max_row=n_data_rows + 2)
...
def write_log_text(
    ...
    """
    Append new poll rows to per-device, per-table columnar text log files.

    Layout
    ------
    Line 1 (written once when file is created):
        Timestamp_Local  | <param1> (<unit1>) | <param2> (<unit2>) | ...
    Subsequent lines (one per poll cycle):
        2026-05-12 08:30:00 | 120.1 | 119.8 | ...

    Only rows that have NOT yet been written are appended on each call,
    making this safe to call multiple times without duplication.  The row
    count already in the file is determined by line counting.
    ...
    for ip, tables in snapshot.items():
        ...
        for tname, tdata in tables.items():
            ...
            filename = os.path.join(log_dir, f"ABMeter_{safe_ip}_{tname}.log")

            # Determine how many data rows already exist (skip header line = 1)
            existing_rows = 0
            ...
            if not is_new:
                try:
                    ...
                    existing_rows = max(0, len(lines) - 1)
                except Exception:
                    existing_rows = 0
                    is_new = True
            ...
            try:
                with open(filename, "a", encoding="utf-8") as fh:
                    if is_new:
                        # Write header: Timestamp_Local | param (unit) | ...
                        col_headers = ["Timestamp_Local"] + [
                            f"{p} ({units_map.get(p, '')})" if units_map.get(p) else p
                            for p in params
                        ]
                        fh.write(" | ".join(col_headers) + "\n")

                    # Append only rows not yet persisted
                    for i in range(existing_rows, len(timestamps)):
                        ts  = timestamps[i]
                        row = [ts] + [
                            str(columns[p][i]) if (p in columns and i < len(columns[p]) and columns[p][i] is not None) else ""
                            for p in params
                        ]
                        fh.write(" | ".join(row) + "\n")

                logger.debug("Log written (columnar): %s", filename)
            except Exception as exc:
                logger.error("Log write failed for %s / %s: %s", ip, tname, exc)
    ...
    for ip, tables in all_device_data.items():
        ...
        try:
            ...
            for tname, tdict in tables.items():
                series = extract_numeric_series(tdict)
                if not series:
                    logger.debug("Veusz: no numeric data in '%s' — skipping", tname)
                    continue
                all_series[tname] = series

            if not all_series:
                logger.warning("Veusz: no numeric data for device %s — skipping", ip)
                doc.Close()
                continue
            ...
            for tname, series in all_series.items():
                ...
                for param, (val, unit) in series.items():
                    ...
                    # Annotation text: encode param + ip into the y-axis label
                    # (graph has no title property; axis label is the cleanest
                    # single-line description visible on the rendered plot).
                    ...
                    # 'label' is a plotter widget added to the graph.
                    # Its 'label' setting is the displayed text string.
                    # xPos/yPos are fractional (0–1) within the graph.
                    lbl = graph.Add("label", name="title_lbl", autoadd=False)
                    lbl.label.val = f"{param} ({ip})"
                    lbl.xPos.val  = [0.5]   # horizontally centred
                    lbl.yPos.val  = [1.02]  # just above the plot area

            # ---------------------------------------------------------------
            # Step 3 — Overlay pages (same new-style API).
            #
            # One page per unit group; all parameters sharing the same unit
            # are overlaid on a single graph.  A 'key' widget provides the
            # legend.  The key widget does have a 'title' setting.
            #
            # xy.key.val = '<param name>'  sets the legend entry text.
            # ---------------------------------------------------------------
            for group_label, substrings in VEUSZ_OVERLAY_GROUPS.items():
                ...
                # --- Single graph (no grid needed for overlays) ---
                ov_graph = ov_page.Add("graph", name="overlay_graph", autoadd=False)
                ...
                for ds_name, idx_name, param, unit in overlay:
                    ...
                    xy.xData.val          = idx_name
                    xy.yData.val          = ds_name
                    xy.key.val            = param    # legend entry text
                    xy.marker.val         = "circle"
                    xy.PlotLine.width.val = "1.5pt"

                # --- label widget for overlay page annotation ---
                ov_lbl = ov_graph.Add("label", name="title_lbl", autoadd=False)
...
def flush_outputs_parallel(cfg: Dict[str, Any]) -> "concurrent.futures.Future":
    ...
    def _do_flush() -> None:
        ...
        if cfg.get("enable_fits"):
            tasks.append(("FITS",  lambda: write_fits(ALL_DEVICE_DATA, fits_dir)))
        if cfg.get("enable_csv"):
            tasks.append(("CSV",   lambda: write_csv(ALL_DEVICE_DATA, csv_dir, append=True)))
        if cfg.get("enable_xlsx"):
            tasks.append(("XLSX",  lambda: write_xlsx(ALL_DEVICE_DATA, xlsx_dir)))
        if cfg.get("enable_log_append"):
            tasks.append(("LOG",   lambda: write_log_text(ALL_DEVICE_DATA, log_dir)))
    ...
    ex = concurrent.futures.ThreadPoolExecutor(max_workers=1, thread_name_prefix="ab_flush_mgr")
...
def launch_gui(
    ...
    class MainWindow(QMainWindow):
        ...
        def _build_output_group(self) -> QGroupBox:
            ...
            self._cb_veusz.setChecked(bool(self._switches.get("enable_veusz",    ENABLE_VEUSZ)))
        ...
        def _do_stop(self) -> None:
            ...
            if cfg.get("enable_veusz") and ALL_DEVICE_DATA:
                ...
                self._append_log("Writing final Veusz file with all accumulated samples…")
                try:
...
def _install_signal_handlers() -> None:
    ...
    def _handler(signum, frame):  # noqa: ANN001
        logger.info("Signal %s received — requesting clean stop after current poll.", signum)
        _HEADLESS_STOP.set()
...
def run_headless(cfg: Dict[str, Any]) -> None:
    ...
    try:
        while True:
            ...
            if _HEADLESS_STOP.is_set():
                logger.info("Stop flag set — exiting loop after cycle %d.", cycle)
                break
            if os.path.exists(STOP_SIGNAL_FILE):
                logger.info("Stop-signal file detected (%s) — exiting.", STOP_SIGNAL_FILE)
                try:
                    os.remove(STOP_SIGNAL_FILE)
                except OSError:
                    pass
            ...
            if should_flush(cfg):
                if flush_future is None or flush_future.done():
                    ...
                    logger.warning("Previous flush still running — skipping mid-cycle flush.")
    ...
    logger.info("Headless loop ended after %d cycle(s). Writing final outputs…", cycle)
    ...
    if cfg.get("enable_veusz"):
        logger.info("Building Veusz project with all %d accumulated sample(s)…", cycle)
        write_veusz(ALL_DEVICE_DATA, veusz_dir)

    logger.info("All outputs written. Output directory: %s", cfg.get("output_base_dir", OUTPUT_BASE_DIR))
