...
Deps   : PySide6, matplotlib, requests, beautifulsoup4, lxml,
         astropy, openpyxl, veusz, psutil
         (pip, conda, uv, or mamba install each)
    ...
    for ip, tables in snapshot.items():
        ...
        for tname, tdata in tables.items():
            ...
            try:
                with open(filename, "a", encoding="utf-8") as fh:
                    if is_new:
                        ...
                        col_headers = ["Timestamp_Local"] + [
                            f"{p} ({units_map.get(p, '')})" if units_map.get(
                                p) else p
                            for p in params
                        ]
                    ...
                    for i in range(existing_rows, len(timestamps)):
                        ts = timestamps[i]
                        row = [ts] + [
                            str(columns[p][i]) if (p in columns and i < len(
                                columns[p]) and columns[p][i] is not None) else ""
                            for p in params
                        ]
                        fh.write(" | ".join(row) + "\n")
                ...
                logger.error("Log write failed for %s / %s: %s",
                             ip, tname, exc)
        ...
        try:
            ...
            for group_label, substrings in VEUSZ_OVERLAY_GROUPS.items():
                ...
                ov_graph = ov_page.Add(
                    "graph", name="overlay_graph", autoadd=False)
